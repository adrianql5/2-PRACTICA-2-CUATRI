...
//Funcion que define los vertices de los ejes
void myEjes() {
...
void dibujarElipse(float radioMayor, float radioMenor) {
	...
	for (int i = 0; i < n; i++) {
		...
		float x = radioMayor * cos(angulo);
		float z = radioMenor * sin(angulo);
		glVertex3f(x, 0, z);
	}
	glEnd();
...
//Funcion que define una lista para representar a saturno con su anillo
int mySaturno(GLfloat tam) {
	...
	//pinto el anillo ahora que ha sido rotado
	dibujarElipse(tam/10, tam/9 );
	...
	glPopMatrix();
    glEndList(); //termino la definicion de la lista y devuelvo el indice
    return indice;
}
